...
object FeatureConstruction {
  ...
  // Aggregate feature tuples from wordCount with COUNT aggregation,
  def constructWordCountFeatureTuple(wordCount: RDD[Count]): RDD[FeatureTuple] = {
    wordCount.map { case Count(subject_id, word, count, charttime) => ((subject_id, word), count.toDouble) }.reduceByKey(_ + _)
  }
