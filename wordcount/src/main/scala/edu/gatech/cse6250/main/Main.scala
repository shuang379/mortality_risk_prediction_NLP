...
import edu.gatech.cse6250.model.{ Diagnosis, LabEvent, Medication, Dead, Alive, Admission, Mortality, Count }
...
object Main {
  def main(args: Array[String]) {
    ...
    val (admissionsRDD, diagnosesRDD, medicationRDD, labEventsRDD, mortalityRDD, wordCountRDD) = loadRddRawData(spark)
    ...
    val (diagFilteredRDD, medFilteredRDD, labFilteredRDD, wordCountFilteredRDD) = filterEvents(admissionsRDD, diagnosesRDD, medicationRDD, labEventsRDD, wordCountRDD)
    ...
    val featureTuples = sc.union(
      ...
      FeatureConstruction.constructWordCountFeatureTuple(wordCountRDD)
    )
    ...
    model_logistic.save(sc, "output/wordcount_logistic")
    ...
    println(s"Accuracy (Word Count, Logistic Regression) = $accuracy_logistic")
    val confusion_logistic = metrics_logistic.confusionMatrix
    println("Confusion matrix (Word Count, Logistic Regression):" + confusion_logistic)
    val AUROC_logistic = metrics_logistic_.areaUnderROC
    println("Area under ROC (Word Count, Logistic Regression) = " + AUROC_logistic)
    val wordcount_logistic = Array(accuracy_logistic, AUROC_logistic, confusion_logistic(0, 0), confusion_logistic(0, 1), confusion_logistic(1, 0), confusion_logistic(1, 1))
    sc.parallelize(wordcount_logistic).saveAsTextFile("output/wordcount_logistic/result")
    ...
    model_decisiontree.save(sc, "output/wordcount_decisiontree")
    ...
    println(s"Accuracy (Word Count, Decision Tree) = $accuracy_decisiontree")
    val confusion_decisiontree = metrics_decisiontree.confusionMatrix
    println("Confusion matrix (Word Count, Decision Tree):" + confusion_decisiontree)
    val AUROC_decisiontree = metrics_decisiontree_.areaUnderROC
    println("Area under ROC (Word Count, Decision Tree) = " + AUROC_decisiontree)
    val wordcount_decisiontree = Array(accuracy_decisiontree, AUROC_decisiontree, confusion_decisiontree(0, 0), confusion_decisiontree(0, 1), confusion_decisiontree(1, 0), confusion_decisiontree(1, 1))
    sc.parallelize(wordcount_decisiontree).saveAsTextFile("output/wordcount_decisiontree/result")
    ...
    model_boostingtree.save(sc, "output/wordcount_boostingtree")
    ...
    println(s"Accuracy (Word Count, Boosting Tree) = $accuracy_boostingtree")
    val confusion_boostingtree = metrics_boostingtree.confusionMatrix
    println("Confusion matrix (Word Count, Boosting Tree):" + confusion_boostingtree)
    val AUROC_boostingtree = metrics_boostingtree_.areaUnderROC
    println("Area under ROC (Word Count, Boosting Tree) = " + AUROC_boostingtree)
    val wordcount_boostingtree = Array(accuracy_boostingtree, AUROC_boostingtree, confusion_boostingtree(0, 0), confusion_boostingtree(0, 1), confusion_boostingtree(1, 0), confusion_boostingtree(1, 1))
    sc.parallelize(wordcount_boostingtree).saveAsTextFile("output/wordcount_boostingtree/result")
  ...
  def filterEvents(admissions: RDD[Admission], diagnoses: RDD[Diagnosis], medication: RDD[Medication], labEvents: RDD[LabEvent], wordCount: RDD[Count]): (RDD[Diagnosis], RDD[Medication], RDD[LabEvent], RDD[Count]) = {
    ...
    val wordKeyValueRDD = wordCount.map { case Count(subject_id, word, count, charttime) => (subject_id, (word, count, LocalDate.parse(charttime.toString, DateTimeFormatter.ofPattern("yyyy-MM-dd")))) }
    ...
    val wordCountFilteredRDD = wordKeyValueRDD.join(observDatesKeyValueRDD).filter { case (subject_id, ((word, count, charttime), (startDate, endDate))) => startDate.isBefore(charttime) && charttime.isBefore(endDate) }.map { case (subject_id, ((word, count, charttime), (startDate, endDate))) => Count(subject_id, word, count, localToSqlDateParser(charttime.toString)) }

    (diagFilteredRDD, medFilteredRDD, labFilteredRDD, wordCountFilteredRDD)
  }

  def loadRddRawData(spark: SparkSession): (RDD[Admission], RDD[Diagnosis], RDD[Medication], RDD[LabEvent], RDD[Mortality], RDD[Count]) = {
    ...
    // wordCount RDD
    val wordCountDFCsv = spark.read.format("com.databricks.spark.csv").option("header", "true").option("mode", "DROPMALFORMED").option("delimiter", ",").load("data/wordCount.csv")
    val wordCountDFCsv_clean = wordCountDFCsv.filter(wordCountDFCsv("charttime") =!= "nan")
    val wordCount: RDD[Count] = wordCountDFCsv_clean.rdd.map { row =>
      new Count(row.getString(0), row.getString(1), row.getString(2).toInt, sqlDateParser(row.getString(3)))
    }

    // val mortality: RDD[Diagnosis] = spark.sparkContext.emptyRDD

    (admissions, diagnoses, medication, labEvents, mortality, wordCount)
  }
