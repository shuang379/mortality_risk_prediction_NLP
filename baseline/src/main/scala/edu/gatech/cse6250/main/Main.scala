...
object Main {
  def main(args: Array[String]) {
    ...
    val (admissionsRDD, diagnosesRDD, medicationRDD, labEventsRDD, mortalityRDD) = loadRddRawData(spark)
    ...
    val (diagFilteredRDD, medFilteredRDD, labFilteredRDD) = filterEvents(admissionsRDD, diagnosesRDD, medicationRDD, labEventsRDD)
    ...
    model_logistic.save(sc, "output/baseline_logistic")
    ...
    println(s"Accuracy (Baseline, Logistic Regression) = $accuracy_logistic")
    val confusion_logistic = metrics_logistic.confusionMatrix
    println("Confusion matrix (Baseline, Logistic Regression):" + confusion_logistic)
    val AUROC_logistic = metrics_logistic_.areaUnderROC
    println("Area under ROC (Baseline, Logistic Regression) = " + AUROC_logistic)
    val baseline_logistic = Array(accuracy_logistic, AUROC_logistic, confusion_logistic(0, 0), confusion_logistic(0, 1), confusion_logistic(1, 0), confusion_logistic(1, 1))
    sc.parallelize(baseline_logistic).saveAsTextFile("output/baseline_logistic/result")
    ...
    model_decisiontree.save(sc, "output/baseline_decisiontree")
    ...
    println(s"Accuracy (Baseline, Decision Tree) = $accuracy_decisiontree")
    val confusion_decisiontree = metrics_decisiontree.confusionMatrix
    println("Confusion matrix (Baseline, Decision Tree):" + confusion_decisiontree)
    val AUROC_decisiontree = metrics_decisiontree_.areaUnderROC
    println("Area under ROC (Baseline, Decision Tree) = " + AUROC_decisiontree)
    val baseline_decisiontree = Array(accuracy_decisiontree, AUROC_decisiontree, confusion_decisiontree(0, 0), confusion_decisiontree(0, 1), confusion_decisiontree(1, 0), confusion_decisiontree(1, 1))
    sc.parallelize(baseline_decisiontree).saveAsTextFile("output/baseline_decisiontree/result")
    ...
    model_boostingtree.save(sc, "output/baseline_boostingtree")
    ...
    println(s"Accuracy (Baseline, Boosting Tree) = $accuracy_boostingtree")
    val confusion_boostingtree = metrics_boostingtree.confusionMatrix
    println("Confusion matrix (Baseline, Boosting Tree):" + confusion_boostingtree)
    val AUROC_boostingtree = metrics_boostingtree_.areaUnderROC
    println("Area under ROC (Baseline, Boosting Tree) = " + AUROC_boostingtree)
    val baseline_boostingtree = Array(accuracy_boostingtree, AUROC_boostingtree, confusion_boostingtree(0, 0), confusion_boostingtree(0, 1), confusion_boostingtree(1, 0), confusion_boostingtree(1, 1))
    sc.parallelize(baseline_boostingtree).saveAsTextFile("output/baseline_boostingtree/result")
  ...
  def filterEvents(admissions: RDD[Admission], diagnoses: RDD[Diagnosis], medication: RDD[Medication], labEvents: RDD[LabEvent]): (RDD[Diagnosis], RDD[Medication], RDD[LabEvent]) = {
    ...
    val aliveObservDatesRDD = eventsDateUnionRDD.reduceByKey((date1, date2) => if (date1.before(date2)) date2 else date1).map {
    ...
    (diagFilteredRDD, medFilteredRDD, labFilteredRDD)
  }

  def loadRddRawData(spark: SparkSession): (RDD[Admission], RDD[Diagnosis], RDD[Medication], RDD[LabEvent], RDD[Mortality]) = {
    ...
    (admissions, diagnoses, medication, labEvents, mortality)
  }
