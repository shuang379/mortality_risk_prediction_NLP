...
object FeatureConstruction {
  ...
  // Aggregate feature tuples from wordCount with AVERAGE aggregation,
  def constructWordFeatureTuple(tfIdf: RDD[TfIdf]): RDD[FeatureTuple] = {
    tfIdf.map { case TfIdf(subject_id, word, tfIdf, charttime) => ((subject_id, word), tfIdf) }.aggregateByKey((0.0, 0))(
      (acc, value) => (acc._1 + value, acc._2 + 1),
      (part1, part2) => (part1._1 + part2._1, part1._2 + part2._2))
      .map { case ((subject_id, itemid), (sum, count)) => ((subject_id, itemid), sum / count) }
  }
